...
def predict():
    ...
    if request.method == 'POST':
        ...
        db_vals = list(output_file.values())
        ...
        if int(result) == 0:
            ...
            output = "Genuine"
        cur=mysql.connection.cursor()
        cur.execute('insert into tests(username,castes,degrees,employs,incomes,mothers,occs,religions,prediction,status) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',(session['username'],db_vals[0],db_vals[1],db_vals[2],db_vals[3],db_vals[4],db_vals[5],db_vals[6],output,"success"))
        mysql.connection.commit()
        cur.close()
        return redirect(url_for('tests'))
    return render_template('test.html',url=url,data = session['username'],tests = tests,castes=castes,degrees=degrees,employs=employs,incomes=incomes,mothers=mothers,occs=occs,religions=religions)
...
def tests():
    cur=mysql.connection.cursor()
    cur.execute("select * from tests where username = %s",([session['username']]))
    tests = cur.fetchall()
    cur.close()
    return render_template('test.html',data = session['username'],url = url, tests = tests,castes=castes,degrees=degrees,employs=employs,incomes=incomes,mothers=mothers,occs=occs,religions=religions)
